<?php
class ReservationAR extends BaseAR
{
	public $word;
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			*/
			...
			array('firstName, lastName, content, phone, checkIn, checkOut, email', 'required'),
			array('phone', 'numerical', 'integerOnly' => true, 'min' => 10),
			array('checkIn, checkOut', 'date', 'format' => 'yyyy-M-d H:m:s'),
			array('content', 'length', 'min' => 20),
			array('email', 'email'),
			array('status, lang', 'safe')
		);
	}
	...
	private function getCriteriaList()
	{
		...
		if(strlen($this->word) > 0){
			$criteria->compare('firstName', $this->word, true, 'OR');
			$criteria->compare('lastName', $this->word, true, 'OR');
			$criteria->compare('phone', $this->word, true, 'OR');
